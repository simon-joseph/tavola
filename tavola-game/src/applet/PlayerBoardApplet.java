...
public class PlayerBoardApplet extends JApplet implements Runnable,
    ...
    public void run() {
	while (true) {//!server.isGameOver()) { //TODO to serwer stwierdza, ze jest juz gameover
	    if (SnakeBoardApplet.clicked) {
		// TODO pobierz ruchy od serwera i wywolaj setSnakesDiractions, setBonus
		playerBoard.update();
		requestFocus();
		board.repaint();
	    }
		...
		// TODO: predkosc tez serwer ustala
		 Thread.sleep(playerBoard.INITIAL_MOVE_TIME
			- speed);
