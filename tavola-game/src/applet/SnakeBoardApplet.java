...
public class SnakeBoardApplet extends JApplet implements Runnable {
  ...
  private SnakeBoard snakeBoard = new SnakeBoard();
  ...
  public SnakeBoardApplet() {
    ...
    new Thread(this).start(); // FIXME wolałbym jeśli dało by się to gdzie
                              // indziej zrobić
  }
  ...
  public void run() {
    ...
    while (!snakeBoard.isGameOver()) {
      ...
      try {
        Thread.sleep(snakeBoard.INITIAL_MOVE_TIME - snakeBoard.getSpeed());
